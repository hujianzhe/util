...
__declspec_dll int mathVertexIndicesFindMinMaxXYZ(const float(*v)[3], const unsigned int* v_indices, unsigned int v_indices_cnt, float v_minXYZ[3], float v_maxXYZ[3]);
__declspec_dll int mathVerticesFindMinMaxXYZ(const float(*v)[3], unsigned int v_cnt, float v_minXYZ[3], float v_maxXYZ[3]);
