...
std::map<T, E>& vector_shift_map(const std::vector<std::pair<K, V> >& v, std::map<T, E>& m) {
	for (std::vector<std::pair<K, V> >::const_iterator it = v.begin(); it != v.end(); ++it)
		m.insert(*it);
	return m;
}
/* list */
...
std::map<T, E>& list_shift_map(const std::list<std::pair<K, V> >& l, std::map<T, E>& m) {
	for (std::list<std::pair<K, V> >::const_iterator it = l.begin(); it != l.end(); ++it)
		m.insert(*it);
	return m;
}
/* set */
...
std::map<T, E>& set_shift_map(const std::set<std::pair<K, V> >& s, std::map<T, E>& m) {
	for (std::set<std::pair<K, V> >::const_iterator it = s.begin(); it != s.end(); ++it)
		m.insert(*it);
	return m;
}
/* map */
...
std::vector<std::pair<T, E> >& map_shift_vector(const std::map<K, V>& m, std::vector<std::pair<T, E> >& v) {
	for (std::map<K, V>::const_iterator it = m.begin(); it != m.end(); ++it)
		v.push_back(*it);
	return v;
}
