...
static int reliable_stream_reply_ack(NioSocket_t* s, unsigned int seq) {
	...
	if (!packet || s->m_sendpacketlist.tail == &packet->msg.m_listnode) {
		...
		*(unsigned int*)(ack + 1) = htonl(seq);
		...
		if (res < sizeof(ack)) {
			packet = (Packet_t*)malloc(sizeof(Packet_t) + RELIABLE_STREAM_DATA_HDR_LEN);
			if (!packet) {
				criticalsectionLeave(&s->m_lock);
				s->m_valid = 0;
				return 0;
...
static int reliable_stream_data_packet_handler(NioSocket_t* s, unsigned char* data, int len, const struct sockaddr_storage* saddr) {
	...
	while (offset < len) {
		...
		unsigned int seq;
		if (len - offset < RELIABLE_STREAM_DATA_HDR_LEN) {
			break;
		}
...
NioSocket_t* niosocketSendv(NioSocket_t* s, const Iobuf_t iov[], unsigned int iovcnt, const struct sockaddr_storage* saddr) {
	...
	if (SOCK_STREAM == s->socktype || !s->reliable.enable) {
		unsigned int hdrlen = s->reliable.enable ? RELIABLE_STREAM_DATA_HDR_LEN : 0;
		...
		if (nbytes) {
			...
			for (i = i_off = offset = 0; offset < nbytes; offset += packetlen) {
				...
				packet = (ReliableDgramDataPacket_t*)malloc(sizeof(ReliableDgramDataPacket_t) + RELIABLE_DGRAM_DATA_HDR_LEN + packetlen);
				if (!packet)
					break;
				packet->msg.type = NIO_SOCKET_RELIABLE_PACKET_MESSAGE;
				packet->s = s;
				...
				packet->len = RELIABLE_DGRAM_DATA_HDR_LEN + packetlen;
				...
				while (i < iovcnt) {
					...
					if (iobufLen(iov + i) - i_off > packetlen - copy_off) {
						...
						memcpy(packet->data + RELIABLE_DGRAM_DATA_HDR_LEN + copy_off, iobufPtr(iov + i) + i_off, copy_len);
						i_off += copy_len;
						break;
					}
					else {
			...
			packet = (ReliableDgramDataPacket_t*)malloc(sizeof(ReliableDgramDataPacket_t) + RELIABLE_DGRAM_DATA_HDR_LEN);
			if (!packet)
				return NULL;
			packet->msg.type = NIO_SOCKET_RELIABLE_PACKET_MESSAGE;
			packet->s = s;
			packet->resendtimes = 0;
			packet->data[0] = HDR_DATA | HDR_DATA_END_FLAG;
			packet->len = RELIABLE_DGRAM_DATA_HDR_LEN;
			nioloop_exec_msg(s->m_loop, &packet->msg.m_listnode);
		}
	}
	return s;
...
void niosocketTcpTransportReplace(NioSocket_t* old_s, NioSocket_t* new_s, int new_recvseq, int new_sendseq) {
	...
	do {
		...
		if (new_sendseq > old_s->reliable.m_recvseq)
			break;
		first_sendseq = reliable_stream_sendpacket_firstseq(&old_s->m_sendpacketlist);
		...
		old_recvseq = old_s->reliable.m_recvseq;
		can_replace = 1;
	} while (0);
	...
	if (can_replace) {
		...
		replacemsg->s = new_s;
		replacemsg->recvseq = old_recvseq;
		replacemsg->sendpacketlist = old_sendpacketlist;
...
NioSocket_t* niosocketCreate(FD_t fd, int domain, int socktype, int protocol, NioSocket_t*(*pmalloc)(void), void(*pfree)(NioSocket_t*)) {
	...
	s->reliable.peer_saddr.ss_family = AF_UNSPEC;
	s->reliable.mtu = 1464 - RELIABLE_DGRAM_DATA_HDR_LEN;
	s->reliable.rto = 200;
...
int nioloopHandler(NioLoop_t* loop, NioEv_t e[], int n, long long timestamp_msec, int wait_msec) {
	...
	for (; cur; cur = next) {
		...
		if (NIO_SOCKET_CLOSE_MESSAGE == message->type) {
			...
			List_t sendpacketlist = replacemsg->sendpacketlist;
...
void niomsgHandler(DataQueue_t* dq, int max_wait_msec, void (*user_msg_callback)(NioMsg_t*, void*), void* arg) {
	...
	for (cur = dataqueuePop(dq, max_wait_msec, ~0); cur; cur = next) {
		...
		if (NIO_SOCKET_USER_MESSAGE == message->type) {
			...
			NioSocket_t* s = pod_container_of(message, NioSocket_t, m_netreconnectmsg);
			if (s->net_reconnect_callback) {
				s->net_reconnect_callback(s, s->m_regerrno);
				s->reg_callback = NULL;
			}
		}
	}
