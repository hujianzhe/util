...
enum {
	...
	NIO_SOCKET_RELIABLE_RESEND_MESSAGE
};
		...
		*(unsigned int*)(ack + 1) = seq;
		...
		if (seq == s->m_recvseq) {
			for (cur = s->m_recvpacketlist.head; cur; cur = next) {
				...
				packet_seq = packet->offset;
				if (packet_seq != s->m_recvseq)
					break;
				next = cur->next;
				s->m_recvseq++;
				msgptr = NULL;
			...
			for (cur = s->m_recvpacketlist.head; cur; cur = cur->next) {
				...
				if (packet_seq > seq)
					break;
				else if (packet_seq == seq)
					return 1;
			}
			packet = (Packet_t*)malloc(sizeof(Packet_t) + len - RELIABLE_HDR_LEN);
			if (!packet) {
				s->valid = 0;
				return 0;
			}
...
NioSocket_t* niosocketSend(NioSocket_t* s, const void* data, unsigned int len, const struct sockaddr_storage* saddr) {
	...
	if (s->reliable && s->socktype != SOCK_STREAM) {
		extra_hdr_len = RELIABLE_HDR_LEN;
	}
	...
	packet->offset = 0;
	packet->len = len;
	memcpy(packet->data + extra_hdr_len, data, len);
...
NioSocket_t* niosocketSendv(NioSocket_t* s, Iobuf_t iov[], unsigned int iovcnt, const struct sockaddr_storage* saddr) {
	Packet_t* packet;
	size_t i, nbytes, extra_hdr_len = 0;
	if (!s->valid)
		return NULL;
	if (!iov || !iovcnt)
		return s;
	...
	packet->offset = 0;
	packet->len = nbytes;
	for (nbytes = extra_hdr_len, i = 0; i < iovcnt; ++i) {
...
NioSender_t* niosenderCreate(NioSender_t* sender) {
	...
	if (!dataqueueInit(&sender->m_dq))
		return NULL;
	hashtableInit(&sender->m_sockht,
		sender->m_sockht_bulks, sizeof(sender->m_sockht_bulks) / sizeof(sender->m_sockht_bulks[0]),
		sockht_keycmp, sockht_keyhash);
	sender->initok = 1;
	return sender;
}
...
void niosenderHandler(NioSender_t* sender, long long timestamp_msec, int wait_msec) {
	...
	for (cur = dataqueuePop(&sender->m_dq, wait_msec, ~0); cur; cur = next) {
		...
		if (NIO_SOCKET_REG_MESSAGE == msgbase->type) {
			...
			hashtableReplaceNode(hashtableInsertNode(&sender->m_sockht, &s->m_senderhashnode), &s->m_senderhashnode);
		}
		else if (NIO_SOCKET_CLOSE_MESSAGE == msgbase->type) {
			NioSocket_t* s = pod_container_of(msgbase, NioSocket_t, m_closemsg);
			hashtableRemoveNode(&sender->m_sockht, &s->m_senderhashnode);
			...
			ListNode_t* cur;
			Packet_t* packet = pod_container_of(msgbase, Packet_t, msg);
			unsigned int ack_seq = packet->offset;
			NioSocket_t* s = packet->s;
			free(packet);
			for (cur = s->m_sendpacketlist.head; cur; cur = cur->next) {
				Packet_t* packet = pod_container_of(cur, Packet_t, msg.m_listnode);
				unsigned int packet_seq = packet->offset;
				if (ack_seq < packet_seq)
					break;
				if (packet_seq == ack_seq) {
					listRemoveNode(&s->m_sendpacketlist, cur);
					free(packet);
					break;
				}
		...
		else if (NIO_SOCKET_RELIABLE_RESEND_MESSAGE == msgbase->type) {
			ListNode_t* cur;
			NioSocket_t* s = pod_container_of(msgbase, NioSocket_t, m_sendmsg);
			if (!s->valid)
				continue;
			for (cur = s->m_sendpacketlist.head; cur; cur = cur->next) {
				Packet_t* packet = pod_container_of(cur, Packet_t, msg.m_listnode);
				int res = socketWrite(s->fd, packet->data, packet->len, 0, &packet->saddr);
				if (res < 0 && errnoGet() != EWOULDBLOCK) {
					s->valid = 0;
					break;
				}
			}
