				...
				if (HDR_ACK == hdr_type) {
					...
					*(int*)(ack + 1) = seq;
					...
					if (seq == s->m_recvseq) {
						...
						if (!is_new_packet)
							break;
						packet = (Packet_t*)malloc(sizeof(Packet_t) + res - RELIABLE_HDR_LEN);
						if (!packet) {
							s->valid = 0;
							break;
						}
						...
						packet->s = s;
						packet->offset = seq;
						packet->len = res - RELIABLE_HDR_LEN;
						memcpy(packet->data, buffer + RELIABLE_HDR_LEN, res - RELIABLE_HDR_LEN);
						if (cur)
...
NioSocket_t* niosocketCreate(FD_t fd, int domain, int socktype, int protocol, NioSocket_t*(*pmalloc)(void), void(*pfree)(NioSocket_t*)) {
	...
	if (SOCK_STREAM == socktype)
		...
		s->m_sendmsg.type = NIO_SOCKET_RELIABLE_RESEND_MESSAGE;
...
static void reactorsocket_real_send(NioSocket_t* s, Packet_t* packet) {
	...
	if (SOCK_STREAM != s->socktype || is_empty) {
		if (s->reliable && SOCK_STREAM != s->socktype) {
			...
			*(int*)(packet->data + 1) = htonl(s->m_sendseq);
			s->m_sendseq++;
		}
